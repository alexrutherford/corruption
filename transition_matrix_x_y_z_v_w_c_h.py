...
#################
    """Takes (int) number of each strategy in population, and global parameters and returns hybrid strategy payoff (float)."""
...
def getStatDist(mat):
#################
    '''Takes (nxn) input transition matrix. Calculates stationary distribution of strategies 
    as dominant eigenvector of transition matrix using Power Iteration method. 
...
#################
    ...
    if len(sys.argv)>1:
        readIn=True
	# if values already calculated, call script with any argument to simply
